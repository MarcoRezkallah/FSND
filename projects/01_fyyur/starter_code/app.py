...
def search_venues():
    venueList = Venue.query.filter(Venue.name.ilike(
...
def create_venue_submission():
    ...
    v.genres = json.dumps(f.getlist('genres'))
    v.website = f['website']
...
@app.route('/artists')
def artists():
    artistList = Artist.query.with_entities(Artist.id, Artist.name)
    data = artistList
...
def create_artist_submission():
    ...
    v.genres = json.dumps(f.getlist('genres'))
    v.website = f['website']
    # v.seeking_venue = f['seeking_venue']
    # v.seeking_description = f['seeking_description']
    try:
        db.session.add(v)
        db.session.commit()
...
def create_show_submission():
    ...
    f = request.form
    s = Show()
    s.artist_id = f['artist_id']
    s.venue_id = f['venue_id']
    s.start_time = f['start_time']
    try:
        db.session.add(s)
        db.session.commit()
